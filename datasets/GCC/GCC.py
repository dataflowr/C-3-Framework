...
class GCC(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train":
            ...
            attributes_pt = torch.from_numpy(
                np.array(
                    [
    ...
    def read_image_and_gt(self, index):
        ...
        img_path = os.path.join(
            cfg_data.DATA_PATH + self.file_folder[index],
            "pngs_544_960",
            self.file_name[index] + ".png",
        )

        den_map_path = os.path.join(
            cfg_data.DATA_PATH + self.file_folder[index],
            "csv_den_maps_" + cfg_data.DATA_GT + "_544_960",
            self.file_name[index] + ".csv",
        )
